...
# 2: 9-long list showing how often each card value appears in the sequence
def preprocess_board(flat_board):
    suit_histogram, histogram = [0] * 4, [0] * 9
...
def detect_straight_flush(suit_board):
    ...
    for index, elem in enumerate(suit_board):
        ...
        if next_val == current_val - 1:
            ...
            if index >= fail_index:
                if (index == fail_index and next_val == 9 and
                        suit_board[0] == 14):
                    return True, 9
                break
            contiguous_length = 1
...
# Full House: (5, trips card, pair card)
...
def detect_hand(hole_cards, given_board, suit_histogram,
    ...
    if max_suit >= 3:
        ...
        if max_suit >= 5:
            ...
            return 6, get_high_cards(suit_board)
    ...
    if current_max == 3 and second_max >= 2:
        return 5, max_val, second_max_val
...
def print_results(hole_cards, winner_list, result_histograms):
    ...
    for index, hole_card in enumerate(hole_cards):
        ...
        if hole_card == (None, None):
            print ("(?, ?) : ", round(winning_percentage*100,2), "%")
        else:
            print (hole_card, ": ", round(winning_percentage*100,2), "%")
    print ("Ties: ", round((float(winner_list[0]) / float_iterations)*100,2), "%\n")
    for player_index, histogram in enumerate(result_histograms):
        ...
        for index, elem in enumerate(histogram):
            print (hand_rankings[index], ": ", round((float(elem) / float_iterations)*100,2), "%")
        print()
